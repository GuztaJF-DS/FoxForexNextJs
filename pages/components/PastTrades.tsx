    ...
    <div className={styles.PastTrades}>
        ...
        <div className={styles.TableHeadMask}>.</div>
          ...
          {tradeData.map((data:any,index:number)=>{
            const DateDay=(data.FinalDate!==null)?data.FinalDate.split("T"):data.StartDate.split("T");
            const DateHour=DateDay[1].split(".");
            const Exchange=(data.ExchangeType==true)?t("Exchange-Type-Buy"):t("Exchange-Type-Sell");
            const Profit=(data.Profit!==undefined)?data.Profit:0;
